...
async def get_dashboard_info(user_id: str=Depends(oauth2.require_user)):
    user = User.find_one({"_id": ObjectId(user_id)})
    if user["role"] != "admin":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="You have no permission to get dashboard data!")
    ...
    pipeline = [
        ...
        {'$project': {
            'total_likes': 1,
            'total_views': 1,
            'channels': {'$add': ['$total_tiktoks', '$total_youtubes', '$total_twitters', '$total_facebooks', '$total_instagrams']}
        }}
    ]
    ...
    start_date = datetime.utcnow().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    end_date = datetime.utcnow()
    info["this_month_uploads"] = Video.count_documents({"created_at": {'$gte': start_date, '$lte': end_date}})
    info["this_month_downloads"] = Video.count_documents({"created_at": {'$gte': start_date, '$lte': end_date}, "marketeer": {'$ne': None}})

    performance = []

    for i in range(datetime.utcnow().month, 13):
        start_date = datetime.utcnow().replace(year=datetime.utcnow().year - 1, month=i, day=1, hour=0, minute=0, second=0, microsecond=0)
        end_date = datetime.utcnow().replace(year=datetime.utcnow().year - 1, month=i, day=calendar.monthrange(start_date.year, start_date.month)[1], hour=23, minute=59, second=59, microsecond=99)
        unit_uploads = Video.count_documents({"created_at": {'$gte': start_date, '$lte': end_date}})
        unit_downloads = Video.count_documents({"created_at": {'$gte': start_date, '$lte': end_date}, "marketeer": {'$ne': None}})
        new_marketeers = User.count_documents({"role": "marketeer", "created_at": {'$gte': start_date, '$lte': end_date}})
        performance.append({"uploads": unit_uploads, "downloads": unit_downloads, "marketeers": new_marketeers})

    for i in range(1, datetime.utcnow().month):
        start_date = datetime.utcnow().replace(month=i, day=1, hour=0, minute=0, second=0, microsecond=0)
        end_date = datetime.utcnow().replace(month=i, day=calendar.monthrange(start_date.year, start_date.month)[1], hour=23, minute=59, second=59, microsecond=99)
        unit_uploads = Video.count_documents({"created_at": {'$gte': start_date, '$lte': end_date}})
        unit_downloads = Video.count_documents({"created_at": {'$gte': start_date, '$lte': end_date}, "marketeer": {'$ne': None}})
        new_marketeers = User.count_documents({"role": "marketeer", "created_at": {'$gte': start_date, '$lte': end_date}})
        performance.append({"uploads": unit_uploads, "downloads": unit_downloads, "marketeers": new_marketeers})
